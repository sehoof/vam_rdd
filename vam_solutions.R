...
mydata <- readstata13::read.dta13("./rdrobust_senate.dta")
...
rdrobust::rdplot(y = y, x = x, binselect = "esmv", title = tit, x.label = "Dem margin of victory in t",
...
rdrobust::rdplot(y = y, x = x, binselect = "es", title = tit, x.label = "Dem margin of victory in t",
       y.label = "Dem vote share in t + 1", 
       x.lim = c(-30, 30), y.lim = c(0, 100))
# Make the cutoff line red
abline(v = 0, lwd = 2, col = 'red')
...
rdrobust>>rdplot(y = y, x = x, binselect = "qsmv", title = tit, x.label = "Dem margin of victory in t",
       y.label = "Dem vote share in t + 1", 
       x.lim = c(-30, 30), y.lim = c(0, 100))
abline(v = 0, lwd = 2, col = 'red')
dev.off()
...
rdrobust::rdplot(y = y, x = x, binselect = "qs", title = tit, x.label = "Dem margin of victory in t",
       y.label = "Dem vote share in t + 1", 
       x.lim = c(-30, 30), y.lim = c(0, 100))
abline(v = 0, lwd = 2, col = 'red')
dev.off()
...
mydata <- mydata %>% dplyr::mutate(treated = ifelse(margin >= 0, 1, 0))
...
mydata <- mydata %>% dplyr::mutate(margin2 = margin * margin)
pooling <- lm(vote ~ margin*treated + margin2 * treated, data = mydata)
summary(pooling)

# Same for cubic
mydata <- mydata %>% dplyr::mutate(margin3 = margin * margin * margin)
pooling <- lm(vote ~ margin*treated + margin2*treated + margin3 * treated, data = mydata)
summary(pooling)

# Same for quartic
mydata <- mydata %>% dplyr::mutate(margin4 = margin2 * margin2)
...
mydata15 <- mydata %>% dplyr::filter(margin <= 15 & margin >= 0)
...
mydata15 <- mydata %>% dplyr::filter(margin >= -15 & margin <= 15)
...
mydata <- mydata %>% dplyr::mutate(weight = ifelse((1 - abs(margin)/15) > 0, (1 - abs(margin)/15), 0))
# Have a look at the weight for each margin
mydata %>% dplyr::select(margin, weight) %>% View()
...
c <- 0
rd <- rdrobust::rdrobust(y, x, c, h = 15, kernel = "triangular", p = 1)
summary(rd)
detach(mydata)
...
# Define cut-off (though if it is zero, you actually do not have to, because it is the default)
c <- 0
rd <- rdrobust::rdrobust(y, x, c, h = 15, kernel = "triangular", p = 1, bwselect = "mserd")
summary(rd)
...
rd <- rdrobust::rdrobust(y, x, c, h = 15, rho = 1, kernel = "triangular", p = 1, bwselect = "mserd")
summary(rd)
...
rd <- rdrobust::rdrobust(y, x, c, h = 15, kernel = "uniform", p = 1, bwselect = "mserd")
summary(rd)
...
y_new <- population
rd <- rdrobust::rdrobust(y_new, x, c, h = 15, rho = 1, kernel = "triangular", p = 1, bwselect = "mserd")
summary(rd)
...
mydata %>% dplyr::select(margin) %>% dplyr::filter(treated == 0) %>% summary()
mydata %>% dplyr::select(margin) %>% dplyr::filter(treated == 1) %>% summary()

# Define placebo cut-off:
c_placebo <- -14.34493
rd <- rdrobust::rdrobust(y,x,c_placebo, h=15, rho=1, kernel="triangular", p=1, bwselect="mserd")
summary(rd)
...
attach(mydata)
rdens <- rddensity::rddensity(margin)
summary(rdens)
detach(mydata)
